...
The optional application client: publish/subscribe, RPC and services over the local runtime socket. Optional by design — an existing application is exposed with `ddrop expose` and never imports this. Transport credentials stay in the runtime, not in your process.
