...
**The era is remembered on disk, and it has to be.** Enrollment is fire-and-forget on start, so a peer that came back up after a rotation would otherwise spend its whole first pass sealing under the secret-derived era, which is readable by exactly the peer that rotation removed. The counter would reset with it, so the replay above would only have to wait for a restart. `<dataDir>/<workspace>.era` holds both, mode 0600, with no config field naming it, on the same reasoning as the identity. It holds symmetric key material and that is not a new exposure: `<workspace>.identity` sits beside it and can unwrap every era ever wrapped for this peer, so anything that can read one can already obtain the other. An era file that will not parse is discarded rather than fatal, which is the opposite of how a corrupt identity is treated, because an unreadable identity is unrecoverable while an unreadable era costs one enrollment pass to rebuild from the store.
