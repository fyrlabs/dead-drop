...
A read-only view carries a much smaller capability, and it is a capability the browser's user already holds by virtue of being able to run `ddrop status`. Keeping the dashboard read-only is what preserves that argument, so it is a constraint of this decision and not a first-release simplification. **A dashboard that could cancel or retry a job would be a control surface reachable from a browser, and it would need this ADR reopened, not extended.**
...
- Anything with local access can read workspace metadata (peer names, channel names, queue depths) from the port for as long as it is open. That is metadata the store already carries in the clear in its object keys (invariant 9), so it is not a new disclosure, but it is a disclosure over a new channel and the docs say so.
