...
# Usage
#   scripts/e2e.sh fast
#   scripts/e2e.sh live <owner/repo>
#   scripts/e2e.sh all <owner/repo>
#   scripts/e2e.sh fast --npm 0.2.5      test a published version, not this tree
#   scripts/e2e.sh fast --only broadcast run scenarios whose file name matches
#   scripts/e2e.sh --list                show the scenarios in each tier
#
...
if [ -z "$TIER" ]; then
  echo "usage: scripts/e2e.sh <fast|live|all> [<owner/repo>] [--npm <version>] [--only <name>]" >&2
  exit 2
fi
if { [ "$TIER" = live ] || [ "$TIER" = all ]; } && [ -z "$REPO" ]; then
  echo "e2e: the live tier needs a repository: scripts/e2e.sh $TIER <owner/repo>" >&2
  exit 2
fi
